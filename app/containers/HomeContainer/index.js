...
export function HomeContainer({
  ...
  useEffect(() => {
    ...
    const loaded = _.get(reposData, 'results', null) || reposError;
    if (loading && loaded) {
      setLoading(false);
    }
  }, []);
  ...
  const renderRepoList = () => {
    const items = _.get(reposData, 'results', []);
    const totalCount = _.get(reposData, 'resultCount', 0);
    return (
              ...
              {items.map((item, index) => (
                <Col xl={{ span: 6 }}>
                  <CustomCard key={index}>
                    <div>
                      <img src={item.artworkUrl100} width="200px" height="150px" />
                    </div>
                    <div>{item.collectionArtistName}</div>
  ...
  const renderErrorState = () => {
    ...
    } else if (!_.get(reposData, 'resultCount', 0)) {
